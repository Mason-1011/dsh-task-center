...
- [x] 抽取层·片 7a「进度第一层·闲置显示连接」——2026-08-18(设计见 [06-extraction.md](06-extraction.md) §7 第一层):闲置口径再进化 —— `effectiveIdle` 增第 4 参 `holderActivity` 连接器,每条记录的触点取 `max(台账 workedAt, 持有会话最后事件时间)`,持有会话在动 = 这条线没被搁下,零账本写入、不撞 CAS;`lastSessionActivity` 跳过 `session/end-seed`(挂载时打的账面标记,恢复会话不能因被挂载显得新鲜);会话不在进程内(已死/未启动)读 undefined 退回 workedAt,只会更新鲜、永不变陈。归属:`dsh-task-center-task` 的 `idle.ts`(纯函数)+ 面板(command-task)与看板(task-web)各自经 `ctx.sessions.get` 提供连接 —— 纯展示 join 不进 task-source,7b/7c(产生账本事件)才住那边;task-web composition 测试随之补挂 dsh-session;keyless 4 纯函数测 + 面板双持对照测(活持有不显示闲置、死持有照常 4 天)+ 全套 131 绿
...
- [x] 抽取层·片 6e「验收档:goal 完成而无人回应」——2026-08-19(设计见 [06-extraction.md](06-extraction.md) §5;起因:贪吃蛇会话 goal 做完后无声消失,用户定案"做完但未经验收的放待验收区、直接进任务待验收"):`foldUnverifiedCompletions`——goal 最新变更 complete 且其后无任何人类消息(`source.kind === 'user'`,后来任何一行都算人回来过)即未验收完成,edit/resume/clear 移出 standing、回应后重完成再计;域动词 `acceptanceCreate`(仅 source;create 带 completionNote 的唯一合法通道)直接生 review 任务——无持有者、acceptance 留空、pack 首行 SUBMITTED,origin `{sessionId, goalId}` 同源去重(TASK_DUPLICATE_ORIGIN;TaskOrigin 第二臂),approve → done、reject → 回可认领 todo(unheld active 无人能认领);触发四门不对称——即时结构通道**只读不生不覆盖**(完成落地时人未及回应),闲置/处置/史扫三门看定局日志再生(处置即生:闭合日志永无人回应);队列加 acceptance 条目(零模型、不吃 summariesPerTick、抛错重入队);marks 域加 `flags` 表(版本不动)承载一次性迁移——首扫零模型重读已覆盖地(贪吃蛇存量靠它浮出),已闲置当场生、未到门入队,跑完落 `acceptance-births` 标记永不重跑;补一个真修复:史扫里已闲置的结构会话生完验收不落覆盖标记,每次重启重读重警告;keyless 16 测 + 全套 169 绿
- [x] 抽取层·片 6f「打回回流:reject 推回来源会话」——2026-08-19(用户指示"打回后应该自动在之前的会话中推送用户输入的理由,让原对话认领";设计见 [06-extraction.md](06-extraction.md) §5 打回回流):`TaskChanged` 增携提交变更原文 `mutation`(理由等字段不在视图投影里,监听方直接读);reject 且 origin 为验收出生(`goalId` 臂)时 `pushRejection`——已搁置来源会话经 `agents.resume` 从持久层恢复、活的经 `agents.get` 复用,先代其认领(claim 回 active,上下文包注入先于消息),再以 `source.kind === 'user'` 用户消息送入打回理由(看板是人的另一张嘴;推送文本指路 task_report 重交),驱动一个回合后 dispose,消息永留日志、路由失败停在日志里等下次打开;重做完成走既有纪律(task_report 提交或新 goal 完成经 7c 镜像;被打回的 goal 已 complete 终态不能回退);两道配套——foldUnverifiedCompletions 的"已回应"序再加 task/change 提交回执(镜像已递进台账的完成不再算未验收,防重做 goal 的镜像提交后闲置门再生二胎),回合结算改按会话串行成链(settling map)且处置终读先等链再折日志(镜像提交回执不与终读赛跑);顺带修一个潜伏问题:reflow 测试配额码字面量与规范常量不符(QUOTA_EXCEEDED vs QUOTA),配额跳过从未真正触发、断言一直靠结算中途竞态通过,改用 QUOTA_EXCEEDED_CODE 并等结算落定再断言;开机对账兜住送达缺口——监听器只听得见活提交,版本切换前的打回与提交/推送间崩溃的裁决靠它重推(推送成功必经认领带出 todo,「验收出生 + 无持有者 todo + 账本末变更 reject」即未送达;task 服务加 `changes(taskId)` 账本历史读,理由只在变更原文里);keyless:mutation 传递、changes() 历史、fold 提交回执、持久栈闭环两型(活打回→恢复会话被代领、理由达模型路由、注入先于消息、无幻影重提、无二胎;版本差打回→重启对账按账本理由重推)+ 配额真断言;补二次打回——fold 打回保留持有者,持有轮的重做本就是该会话的,推送跳过重领,回执日志上再推一轮;全套 175 绿
- [x] 看板·「历史对话」——2026-08-19(用户指示「任务除了持有会话还要有历史对话,看板上的会话支持点击直达会话页」):`historySessionIds(record)` 纯派生补上「验收/晋升出生的 origin 会话从不出现在 sessionIds」的缺口——origin 会话置首、认领史按首认顺序、同会话再认领只列一次(真账本首例:打回代领 + 退回待验收再认领,同 id 两笔 claim),无账本事件变化;三处消费——task-web 看板卡 `historySessions`(空则省略,wire 既有约定)、task_query 投影 `historySessionIds`(必列空数组)、`claim` 注入 content 前置 `PRIOR SESSIONS: <id> …`(去掉本次会话后非空才注入,首个认领者不注入);详情弹窗新增「历史对话」区,持有会话、历史条目、候选卡来源会话均为可点 chip——`ctx.sessions.open` 直达会话页(客户端插件 inject 增 `sessions`,跳转先收起看板浮层);keyless:派生+注入一测(无史不注入、origin 置首、打回释放后再认领累积)、工具投影断言、bundle 断言(overlay face 携 openSession 且真调);全套 176 绿
- [x] 看板·「出生工作区」融合(方案 A:出生盖戳)——2026-08-19(用户指示「项目需要和工作区做一定程度的融合,一个任务默认归属其所在工作区」;评估过整替与注册表引用后定案自含盖戳):`workspaceIds` 数组退场,TaskRecord 换单值可选 `workspacePath`——建任务那一刻盖一次、永不改写,自含路径字符串不引用工作区注册表(账本自校验与人类闸门都不受影响);四个出生口全部盖戳——模型 task_create 从会话头 cwd 盖(非模型参数,事实不是选择)、验收出生带来源会话 cwd(task-source 四门透传)、候选晋升由 task-web 解来源会话目录(活读会话头、死读 sessionPersistence header,都没有则不盖)、人工看板新建无戳;`list`/`task_query` 过滤键随改 workspace_path(精确匹配);看板分组升级为「全部 | 项目 | 工作区 | 无分组」四类 pill——显式项目优先于盖戳(挂了项目的任务不计入工作区组),工作区组只收非空,标签取目录末段(撞名退全路径,title 悬停给全路径);详情弹窗新增「工作区(出生目录)」行;存量任务无戳,不回填(出生是不可伪造的事实);keyless:服务过滤测改 workspacePath、验收出生双断言(无 cwd 不盖、有 cwd 盖上)、工具投影 null 断言(测试会话无 cwd)、task-source 处置出生盖戳断言;全套 176 绿
- [x] 时间面·「定时发送 + 看板唤醒可见 + 阻塞原因标注」——2026-08-19(用户定案「两样都做,且需要支持在会话页面\面板页面能够输入定时发送时间与发送内容(默认 cont);额度感知:阻塞栏特殊标注阻塞原因」):新包 `task-sched`——人定时间与内容(默认 `cont`),到点作为用户消息送进目标会话;发送记录住自己的 storage 域(`task_sched` 域 `sends` 表,调度簿记不进任务账本,与抽取层 marks 同拆分),`pending → firing → fired/failed` 先落 firing 再投递,崩溃卡壳行挂载时交还 pending,记录经全量重挂载仍在;投递复用 task-source 模式(活会话直投、死的 `agents.resume` 带原 preset——`recordedPresetSetup` 随之从 task-source 导出,欠了它恢复会话听不懂工具)、等回合结束即处置;死目标(无活会话又无持久层可恢复)settles failed 带注记可清除;两个人口——会话页头部「⏰ 定时」弹窗(经 dsh web 会话级槽位 `conversation.session.header.actions` + `conversation.input.dock`,标准 kit 携 sessionId)与看板详情「定时发送」栏(目标会话下拉 = 持有者 + 历史会话;通道缺席优雅隐身);部署钉死单运行器——同一张表只装 web profile 一个进程轮询(两个运行器对同一行各投一次),headless/独立壳不装;看板补定时唤醒可见——卡片 `⏰ 标签 · 下次到点` 标记、详情「定时唤醒」行(label/nextAt 经 `describeWake`/`nextWakeAt` 纯函数,与 command-task 共源去重);阻塞栏标注原因类别——卡片与详情 `阻塞·额度: 理由`(`blockedLabel` 中文类别码,未知码原样);keyless:task-sched 10 测(SRC 面标识符纪律、活投递闭环 fired、未到点持有 + 取消、四校验码、死目标 failed、配置大声拒、崩溃恢复 + 重挂载存活、存储序/持久、bundle 冒烟)+ 看板唤醒标记宿主测 + 标签单测;全套 190 绿
