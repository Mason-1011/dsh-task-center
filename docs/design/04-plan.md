...
- [x] 看板·「历史对话」——2026-08-19(用户指示「任务除了持有会话还要有历史对话,看板上的会话支持点击直达会话页」):`historySessionIds(record)` 纯派生补上「验收/晋升出生的 origin 会话从不出现在 sessionIds」的缺口——origin 会话置首、认领史按首认顺序,无账本事件变化;三处消费——task-web 看板卡 `historySessions`(空则省略,wire 既有约定)、task_query 投影 `historySessionIds`(必列空数组)、`claim` 注入 content 前置 `PRIOR SESSIONS: <id> …`(去掉本次会话后非空才注入,首个认领者不注入);详情弹窗新增「历史对话」区,持有会话、历史条目、候选卡来源会话均为可点 chip——`ctx.sessions.open` 直达会话页(客户端插件 inject 增 `sessions`,跳转先收起看板浮层);keyless:派生+注入一测(无史不注入、origin 置首、打回释放后再认领累积)、工具投影断言、bundle 断言(overlay face 携 openSession 且真调);全套 176 绿
