...
| `task/task-source` | Provider(抽取) | 常驻扫描器:闲置/处置触发 → fold 会话日志(goal/计划/todo)产候选,人确认晋升(见 06);进度回流(7a–7c)也住这里 |
...
- `tool-task`、`command-task`、`task-quota`、`task-reaper`、`task-source`、`shell`:消费型函数插件(形态 4);
