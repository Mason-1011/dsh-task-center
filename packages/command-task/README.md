...
**用途一句话**:任务接缝的人类面——一条 `/task` 斜杠命令(项目分组并标闲置天数的面板 / 详情 / 建任务 / 定时唤醒 / 验收 / 打回)加 `/task project` 项目管理,approve、reject 与项目的建改归档只在这里合法。
...
**id 前缀**:按前缀唯一匹配任务 id;歧义时报错并列出候选,不猜。匹配扫全量(含归档),不受列表默认条数限制。**项目键**同理:按项目名前缀或 id 前缀唯一匹配,歧义列出候选。

**面板分组**:项目按创建序排列,已归档项目标注 `· 已归档` 并继续展示其任务(只是不再接收新任务);每组内部再按阻塞 → 待验收 → 在办 → 待办分组;`🗑 无项目` 收尾。

**闲置与搁置告警**:闲置天数 = 距任务最后一次域事件(`updatedAt`)的整天数,不足 1 天不显示;只有未完结(非已完成、非归档)任务计闲置,口径为**子树感知**——自身或任一后代(不论其状态)有更近的动作就算不闲,委派进行中的父任务不会被误报搁置。项目组头与 `/task project` 列表的闲置取组内未完结任务的最长值。最久闲置达到 `staleDays` 时,该任务钉在面板顶部:`⚠ 搁置最久(闲置 N 天)`。
...
- 闲置在面板渲染时计算,不落账本;`updatedAt` 是最后一次域事件时间,人类、模型与机械簿记(如 wake-set)都会刷新它。
