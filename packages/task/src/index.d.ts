...
import type { TaskActor, TaskContextInjectedMeta, TaskDomainEvent, TaskError, TaskId, TaskOperation, TaskSnapshotChangeMeta, TaskView } from './types.ts';
...
export declare class TaskService extends Service {
    ...
    }, actor: TaskActor): Promise<TaskHandle>;
    /** Read one task view. */
    get(taskId: TaskId): Promise<TaskView | undefined>;
    ...
    claim(taskId: TaskId, session: Session, actor: TaskActor): Promise<TaskView | TaskError>;
    /** Single entry point for every transition; compare-and-set on revision. TODO(S1). */
    mutate(taskId: TaskId, expectedRevision: number, change: TaskOperation, actor: TaskActor): Promise<TaskView | TaskError>;
    /** Wake rules that reached their target, for task-wake. TODO(S1). */
    wakeRules(): Promise<readonly WakeDue[]>;
