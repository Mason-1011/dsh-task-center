...
本包是 Typert Remote 服务,暴露两个端点(web 看板头部的「自动续做 开/关」开关就是它们的消费方):

| 端点 | 行为 |
|---|---|
| `quotaGet` | 返回当前生效的旋钮值(上次切换值,否则配置默认值) |
| `quotaSet` | 切换旋钮,立即生效于**下一次**额度墙(已挂起的唤醒规则不回滚);布尔值之外的入参被拒(`QUOTA_INVALID_VALUE`) |
