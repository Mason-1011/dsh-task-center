...
- 唤醒起会话前不做预检探测(preflight probe):到点若额度仍未恢复,新会话会再撞一次墙——届时按同一逻辑重新挂起(每 5 小时窗口最多撞两次),探测滑动留 P1。
- 只处理"终死"的额度失败;带重试插件时瞬时 429 会在 dsh-llm-retry 层被消化,到不了本层(未装重试插件时 QUOTA 即终死,语义不变)。
