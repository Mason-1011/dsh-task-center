...
| 工具 | 作用 | 主要错误码 |
|---|---|---|
| `task_create` | 建任务(objective / acceptance);出生工作区由会话头盖戳(非参数);parent_task_id 挂为父任务子任务,挂接被拒即回收新建任务;project_id 归入人类项目,被拒(缺/归档)则任务不建 | invalid_objective / invalid_acceptance / not_claimed / not_found / invalid_subtask |
...
| `task_query` | 按 status / workspace_path(出生目录精确匹配) / project_id / limit 过滤;parent_task_id 改列该任务的存活子任务 | invalid_filter / not_found |
