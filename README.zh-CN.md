...
- **额度感知**——API 额度用尽自动挂起并释放持有,额度重置点自动唤醒续做(可在 task-quota 配置 `resumeOnReset: false` 关闭,只挂起等人工);阻塞卡片标注阻塞原因类别(额度/人工等);
...
| `resumeOnReset` | task-quota | true | 额度重置点是否自动设唤醒续做;false = 只挂起释放,等人工唤醒 |
